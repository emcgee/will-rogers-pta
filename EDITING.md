...
- **Every page contains its own copy of the header (menu) and footer.** If you change
  the menu or footer, make the same change on *every* `.html` file — or just ask
  Claude, which does this reliably in one step. (The blocks are marked with
  `<!-- ===== Site header ... -->` comments.)
